...
export default function PatientPortalPage() {
  ...
  return (
    <div style={{ background: "var(--cream)", minHeight: "100vh" }}>
      <PortalHeader activePage="overview" basePath="/portal/patient" />
          ...
          <div className="flex flex-col gap-4 sm:gap-5">
            ...
            <Card>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                {[
                  { avatar: "M", from: "Michelle Hernandez, LEP, Ed.S.", subj: "Following up on your evaluation notes", time: "9:42 am", unread: true },
                  { avatar: "R", from: "Rae, Client Coordinator", subj: "Your evaluation report is ready", time: "Yesterday", unread: true },
                  { avatar: "M", from: "Michelle Hernandez, LEP, Ed.S.", subj: "Session summary from Apr 30", time: "May 1", unread: false },
                ].map((msg, i) => (
                  <article key={i} style={{ display: "flex", gap: "0.875rem", padding: "0.875rem 0", borderBottom: i < 2 ? "1px solid var(--linen)" : "none", cursor: "pointer" }}>
                    <div style={{ width: 38, height: 38, borderRadius: "50%", background: "var(--sage-100)", color: "var(--sage-700)", display: "inline-flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-display)", fontSize: "0.95rem", flexShrink: 0 }}>{msg.avatar}</div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: "0.875rem", fontWeight: msg.unread ? 500 : 400, color: msg.unread ? "var(--ink)" : "var(--stone-700)" }}>{msg.subj}</div>
                    </div>
                ...
                {TASKS.map((task, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {task.tag === "urgent" && <span style={{ fontSize: "0.75rem", padding: "0.2rem 0.6rem", borderRadius: 100, background: "rgba(166,60,46,.08)", color: "var(--color-error)", border: "1px solid rgba(166,60,46,.2)", flexShrink: 0 }}>Due soon</span>}
                      {task.tag === "optional" && <span style={{ fontSize: "0.75rem", padding: "0.2rem 0.6rem", borderRadius: 100, background: "var(--sage-50)", color: "var(--sage-600)", border: "1px solid var(--sage-200)", flexShrink: 0 }}>Optional</span>}
                    </li>
                  );
                })}
              </ul>
            ...
            <Card>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "0.5rem", marginTop: "0.75rem" }}>
                <span style={{ fontSize: "0.875rem", color: "var(--stone-700)" }}>
                  Mood trending <em style={{ color: "var(--sage-500)", fontStyle: "italic", fontFamily: "var(--font-display)" }}>gently upward</em>
                </span>
