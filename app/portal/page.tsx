...
export default function PortalPage() {
  ...
  return (
    <div style={{ background: "var(--cream)", minHeight: "100vh" }}>
      ...
      {/* ════════════════ Header ════════════════ */}
      <header style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(248,246,242,.97)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", borderBottom: "1px solid var(--linen)" }}>
        <div
          className="flex items-center"
          style={{ maxWidth: 1320, marginInline: "auto", padding: "0 clamp(1.25rem,4vw,2.5rem)", height: 62, gap: "1.5rem" }}
        >
          {/* Brand */}
          <Link href="/portal" style={{ display: "inline-flex", alignItems: "center", gap: "0.625rem", color: "var(--ink)", fontFamily: "var(--font-display)", fontSize: "0.95rem", textDecoration: "none", flexShrink: 0 }}>
            <span className="brand-mark">M</span>
            <span className="hidden sm:inline">Minded Connections</span>
          </Link>

          {/* Nav links — desktop only */}
          <nav className="hidden lg:flex items-center gap-0.5 flex-1" aria-label="Primary">
            <Link
              href="/portal"
              aria-current="page"
              style={{ display: "inline-flex", alignItems: "center", gap: "0.375rem", padding: "0.45rem 0.75rem", fontSize: "0.875rem", color: "var(--sage-600)", fontFamily: "var(--font-body)", textDecoration: "none", borderRadius: 6, background: "var(--sage-50)" }}
            >
            ...
            {[
              { label: "Appointments", d: <><rect x="3" y="5" width="18" height="16" rx="2" /><path d="M3 9h18" /><path d="M8 3v4M16 3v4" /></> },
              { label: "Messages", d: <path d="M21 11.5a8.4 8.4 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.4 8.4 0 0 1-3.8-.9L3 21l1.9-5.7a8.4 8.4 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.4 8.4 0 0 1 3.8-.9h.5a8.5 8.5 0 0 1 8 8z" />, badge: 2 },
              { label: "Records", d: <><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><path d="M14 2v6h6" /><path d="M9 13h6M9 17h4" /></> },
              { label: "Billing", d: <><rect x="2" y="6" width="20" height="13" rx="2" /><path d="M2 11h20" /></> },
            ].map(({ label, d, badge }) => {
              const NAV_HREFS: Record<string, string> = { Appointments: "/portal/appointments" };
              return (
              <a key={label} href={NAV_HREFS[label] ?? "#"} style={{ display: "inline-flex", alignItems: "center", gap: "0.375rem", padding: "0.45rem 0.75rem", fontSize: "0.875rem", color: "var(--stone-700)", fontFamily: "var(--font-body)", textDecoration: "none", borderRadius: 6 }}>
                ...
                {label}
                {badge && (
                  <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 17, height: 17, borderRadius: "50%", background: "var(--sage-500)", color: "white", fontSize: "0.6875rem", fontWeight: 600, fontFamily: "var(--font-body)", lineHeight: 1 }}>{badge}</span>
                )}
              </a>
            );
            })}
          ...
          {/* Actions — ml-auto keeps them right on mobile when nav is hidden */}
          <div className="flex items-center gap-2 ml-auto">
            <button aria-label="Notifications" style={{ position: "relative", width: 38, height: 38, background: "none", border: "none", borderRadius: 6, display: "inline-flex", alignItems: "center", justifyContent: "center", color: "var(--stone-700)", cursor: "pointer" }}>
            ...
            <button onClick={handleSignOut} title="Sign out" style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", padding: "0.375rem 0.75rem 0.375rem 0.375rem", background: "none", border: "1.5px solid var(--linen)", borderRadius: 100, cursor: "pointer" }}>
              <span style={{ width: 28, height: 28, borderRadius: "50%", background: "var(--sage-100)", color: "var(--sage-700)", fontFamily: "var(--font-display)", fontSize: "0.875rem", display: "inline-flex", alignItems: "center", justifyContent: "center" }}>S</span>
              <span className="hidden sm:inline" style={{ fontSize: "0.875rem", color: "var(--ink)", fontFamily: "var(--font-body)" }}>Sarah C.</span>
            </button>
          </div>
        </div>
      </header>
